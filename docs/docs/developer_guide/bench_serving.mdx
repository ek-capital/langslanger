...
- `speed-bench-qualitative`: LangSlanger's canonical semantic and speculative-decoding workload. It preserves the full SPEED-Bench conversation and covers 11 categories with 80 prompts each. Use a chat backend.
- `speed-bench-throughput`: LangSlanger's canonical system-performance workload. It uses the SPEED-Bench fixed-input-length artifacts (1K, 2K, 8K, 16K, and 32K), each split into `low_entropy`, `mixed`, and `high_entropy` categories.
- `speed-bench`: compatibility alias for `speed-bench-throughput`; prefer the explicit canonical name in new results.
...
- `--dataset-path PATH`: path to a source-complete JSONL artifact produced by the [SPEED-Bench measurement framework](https://github.com/NVIDIA/Model-Optimizer/tree/main/examples/specdec_bench). Raw Hugging Face rows containing source placeholders are rejected.
- `--speed-bench-category`: for `speed-bench-qualitative`, one of `coding`, `humanities`, `math`, `multilingual`, `qa`, `rag`, `reasoning`, `roleplay`, `stem`, `summarization`, or `writing`; for `speed-bench-throughput`, one of `low_entropy`, `mixed`, or `high_entropy` (default: all)
- `--speed-bench-output-len`: fixed number of output tokens per request (default: 512)

For reproducible LangSlanger results, use Hugging Face dataset `nvidia/SPEED-Bench` at revision `487aa718444e816458d1a0a52bfce7a454285cf4` and retain the materialized JSONL SHA-256 emitted in the benchmark result. Synthetic datasets remain useful controls, but they are not canonical semantic workloads.
...
3. To benchmark speculative-decoding acceptance across the qualitative coding category, use a chat backend:
...
python3 -m sglang.bench_serving \
    --backend sglang-oai-chat \
    --dataset-name speed-bench-qualitative \
    --dataset-path /path/to/qualitative.jsonl \
    --speed-bench-category coding \
    --speed-bench-output-len 512 \
    --num-prompts 80
```

4. To profile serving throughput using the mixed-entropy 1K input-length artifact:

```bash Command
python3 -m sglang.bench_serving \
    --backend sglang \
    --dataset-name speed-bench-throughput \
